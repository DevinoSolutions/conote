...
- 335c6bb: Fix input rules crashing when the matched text spans an inline atom node like a mention.
- 335c6bb: Fix `editor.$pos()` returning the wrong node inside container nodes, for example the list item instead of the list.
- @tiptap/pm@3.28.1
...
- 335c6bb: Keep line breaks inside table cells when serializing to markdown. Hard breaks and paragraph breaks in a cell are now written as `<br>` instead of being collapsed into a space, so they survive a parse/serialize round trip.
- Updated dependencies [335c6bb]
- Updated dependencies [335c6bb]
  - @tiptap/core@3.28.1
  - @tiptap/pm@3.28.1
